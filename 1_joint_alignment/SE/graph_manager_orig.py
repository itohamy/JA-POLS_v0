...
class GraphManager:
    ...
    def __init__(self, data):
        ...
        self.create_E()
        self.create_V()
        self.starting_frame = config.se['starting_frame']
    ...
    def transform_images_globally(self, n_frames):
        print('\nTransform Images globally...')
        imgs_trans_all = []
        img_sz = self.data.img_sz
        img_emb_sz = self.data.img_emb_sz
        img_big_emb_sz = self.data.img_big_emb_sz
        ...
        trans = []
        T0_inv = invert_T(self.V[self.starting_frame])
        for v in self.V:
            if v in range(0, n_frames):
                ...
                if np.isfinite(np.linalg.cond(T)):
                    T_SE = invert_T(T)
                    # T_SE = np.eye(3)[0:2, :] # DEBUG: STN WITHOUT SESYNC
                    if T_SE is not None:
    ...
    def prepare_data_for_STN(self, nparray_path):
        ...
        # np.save(nparray_path + "example_img_for_STN_test.npy", self.data.example_img)
